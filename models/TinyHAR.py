...
class TinyHAR_Model(nn.Module):
    def __init__(
        ...
        self.channel_interaction = crosschannel_interaction[cross_channel_interaction_type](input_shape[3], filter_num) # Orignal
        # self.channel_interaction = crosschannel_interaction[cross_channel_interaction_type](input_shape[3], 1) #Without Initial Covnet
        ...
        if cross_channel_aggregation_type == "FC":
            ...
            self.channel_fusion = crosschannel_aggregation[cross_channel_aggregation_type](input_shape[3]*filter_num,2*filter_num) # Orignal
            # self.channel_fusion = crosschannel_aggregation[cross_channel_aggregation_type](input_shape[3],2*filter_num) # Without INitial Covnet
            # self.channel_fusion = crosschannel_aggregation[cross_channel_aggregation_type](70,2*filter_num) # Without Cross channel Interaction - MIT
            # self.channel_fusion = crosschannel_aggregation[cross_channel_aggregation_type](50,2*filter_num) # Without Cross channel Interaction - HUST
    ...
    def forward(self, x):
        ...
        # print("After CNN Subnet", x.shape)
        ...
        # ------->  B x C x F* x L* 
        # print("After Cross Channel Interaction ", x.shape)
        x = self.dropout(x)
        ...
        # print("After Cross Channel Fusion ", x.shape)
        ...
        # print("ross temporal fusion ", x.shape)
