...
import itertools
import pandas as pd
dstes = ['pamap'] #['mhealth', 'pamap'] #['realworld', 'mhealth'] #['pamap', 'wisdm'] #['pamap', 'wisdm']  #,
...
if __name__ == '__main__':
    for dset, iter in itertools.product(dstes, iterations):
        ...
        dset_tr_0 = dataset.load(name=args.dataset, root=pth_dataset, mode='train_0', windowlen= window_len, autoencoderType= None, standardize = True)
        ...
        dset_ev = dataset.load(name=args.dataset, root=pth_dataset, mode='eval_0', windowlen=window_len, autoencoderType= None, standardize = True)
        dlod_ev = torch.utils.data.DataLoader(dset_ev, batch_size=args.sz_batch, shuffle=False, num_workers=args.nb_workers)

        dset_test_0 = dataset.load(name=args.dataset, root=pth_dataset, mode='test_0', windowlen=window_len, autoencoderType= None, standardize = True)
        ...
        dset_tr_now = dataset.load(name=args.dataset, root=pth_dataset, mode=dset_tr_now_md,windowlen= window_len, autoencoderType= None, standardize = True)
        dset_ev_now = dataset.load(name=args.dataset, root=pth_dataset, mode=dset_ev_now_md,windowlen= window_len, autoencoderType= None, standardize = True)
        dset_test = dataset.load(name=args.dataset, root=pth_dataset, mode='test_1', windowlen= window_len, autoencoderType= None, standardize = True)
        ...
        stage_2_acc_0, stage_2_f1_0, acc_n, f1_n, _, _ = get_accuracy_old_new_LSTM(model, dlod_test, nb_classes, nb_classes_now, trained_prototypes_dict)
